...
Read(INP, *)N, L
Write(IPR, 300)N, L
300 Format (//' DATA'//2X, 'NUMBER OF BOUNDARY ELEMENTS =', I3/2X, 'NUMBER OF INTERNAL POINTS WHERE THE FUNCTION IS CALCULATED =', I3)

Write(IPR, 500)
500 Format(//2X, 'COORDINATES OF THE EXTREME POINTS OF THE BOUNDARY ELEMENTS', //1X, 'POINT', 7X, 'X', 15X, 'Y')
...
Write(IPR, 800)
800 Format(//2X, 'BOUNDARY CONDITIONS'//2X, 'NODE', 6X, 'CODE', 7X, 'PRESCRIBED VALUE')
...
KK = J+1
If (I-J)20, 25, 20
20 Call EXTINPC(XM(I), YM(I), X(J), Y(J), X(KK), Y(KK), H(I,J), G(I, J), DQ1, DQ2, DU1, DU2, 0)
Go to 30
...
Call EXTINPC(CX(K), CY(K), X(J), Y(J), X(KK), Y(KK), A, B, DQ1, DQ2, DU1, DU2, 1)
...
Write(IPR, 100)
100 Format(' ', 79('*')//1X, 'Results'//2X, 'Boundary Nodes'//8X, 'X', 15X, 'Y', 13X, 'Potential', 3X, 'Potential Derivative'/)
...
If(L.EQ.0) Go to 30
Write(IPR, 300)
300 Format(//, 2X, 'Internal Points', //8X, 'X', 15X, 'Y', 13X, 'Potential', 9X, 'Flux X', 10X, 'Flux Y'/)
