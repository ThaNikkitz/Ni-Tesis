...
\begin{table}[H]
	...
	\begin{tabular}{cccc}\hline
		...
		vert.$/\angstrom^2$ & \rule{0pt}{15pt}[-] & kcal/mol & kcal/mol \\\hline
...
\begin{table}[H]
	...
	\begin{tabular}{cccccc}\hline
		\multirow{2}{*}{Densidad} & Número de & \multirow{2}{*}{E\_solv (PyGBe)} & E\_solv (PyGBe & Tiempo de & RAM\\
		& elementos &  & + Membrana) & cálculo & utilizada\\\hline
		\rule{0pt}{1pt} vert.$/\angstrom^2$ & [-] & kcal/mol & kcal/mol & [s] & [GB] \\\hline
...
Como en \texttt{APBSmem} hay que definir un dominio de integración rectangular, se opta por un cubo de $128\,\angstrom$ de lado, con tres cantidades de elementos, de modo que cada uno de estos sea de $0.25\,\angstrom, 0.5\,\angstrom$ y $1\,\angstrom$. Se utiliza \texttt{APBS} con mapas dieléctricos modificados para incluir la membrana, y por lo tanto el programa entrega la misma diferencia que \texttt{PyGBE}, pero en unidades de $kJ/mol$. Esto no requiere más que una división por $4.184\, kJ/kcal$ para hacer los resultados directamente comparables. Para la primera prueba, los resultados son los siguientes



La razón de solo presentar dos sets de resultados para la molécula denominada \texttt{3q} es que se repite el comportamiento apreciado en la tabla \ref{subsec:apbs_centered_ion}; los resultados entre las condiciones \texttt{sdh} y \texttt{mdh} varían de forma despreciable. Por esto es que únicamente se presentan los resultados para condiciones \texttt{zero} y \texttt{sdh}.
