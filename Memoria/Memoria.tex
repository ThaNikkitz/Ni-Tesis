...
El método de imágenes presentado permite dar cuenta del efecto de dos interfaces de forma implícita, y esa es su gran fortaleza, pues no se requiere una discretización de la membrana o las interfaces membrana - solvente. En vez de eso, se reproduce el efecto de los campos de reacción en los puntos de cuadratura al modificar las funciones de Green de los operadores integrales, como ya se adelantó. Ahora se ve más detalladamente cómo se trabaja este sistema de forma numérica.\\\\
Para empezar, se requiere la discretización tal cual se indica en el apartado \textcolor{red}{Poisson - Boltzmann}. En dicha sección no se realiza un desarrollo específico, pero ahora se procede a explicar bien cómo se trabaja matemáticamente con BEM. Para un panel de referencia $a$ que actúa como \texttt{src}, se debe calcular la interacción con cada uno de los $N-1$ paneles restantes (con contador $b$), \textbf{y} consigo mismo. El cálculo de los Single y Double Layer Potentials requiere la discretización del panel para el cómputo numérico de las integrales, lo que supone en este caso distribuir $k$ puntos de Gauss sobre dicho panel para calcular los valores de los integrandos en dichos puntos
\begin{align}
Poner V y K para los puntos de Gauss
\end{align}
