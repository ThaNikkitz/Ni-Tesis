...
\begin{picture}(18,10)
\put(0,0){\includegraphics[scale = 1]{./Figures/UTFSM_logo.png}}
\end{picture}
\\
\\
\begin{center}
...
\begin{align}
\frac{\phi}{2} + \int_{\partial\Omega}\frac{\partial}{\partial\hat{\mathbf{n}}}\phi(\mathbf{r})\sum_{i=-n}^{n}\frac{q_i}{4\pi|\mathbf{r} - \mathbf{r}_i^*|} d\partial\Omega - \int_{\partial\Omega}\phi(\mathbf{r})\sum_{i = -n}^{n}\frac{q_i}{4\pi}\nabla\left(\frac{1}{|\mathbf{r} - \mathbf{r}_i^*|}\right)\cdot\hat{\mathbf{n}} d\partial\Omega
\end{align}
...
\subsection{Cálculo numérico de las energías}
...
\subsubsection{Resultados mediante PyGBe}



\subsubsection{Resultados mediante APBS - APBSmem}



\subsection{Extrapolación de Richardson y error relativo}
