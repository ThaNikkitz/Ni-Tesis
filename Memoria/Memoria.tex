...
\chapter{Metodología, funcionamiento e implementación}
...
\end{align*}
Además retienen su significado los términos siguientes
\begin{equation*}
q_i = q\left(\frac{\epsilon_2 - \epsilon_1}{\epsilon_1 + \epsilon_2}\right)^{|i|} \qquad\qquad z_i=(-1)^iz_0 + i\cdot a \qquad\qquad \mathbf{r}_i = (x, y, z_i)
\end{equation*}
Aplicando las condiciones de contorno de electrostática, continuidad del potencial y el salto en la componente normal del campo eléctrico (Ec. (\ref{eqn:jump_condition}, \ref{eqn:pot_continuity})) el sistema de ecuaciones se puede llevar a uno con incógnitas en términos de un único $\phi$ y su derivada $\partial/\partial\hat{\mathbf{n}}(\phi)$. Utilizando la notación de los operadores, se obtiene
\begin{gather}

\end{gather}
La superficie, en este caso SES, se subdivide en $N$ paneles triangulares y las Ecuaciones \ref{eqn:membrane1} y \ref{eqn:membrane2}, tal como en el caso para Poisson - Boltzmann, son colocadas en el centroide de cada panel.

Para un panel de referencia $v$ que actúa como \texttt{src}, se debe calcular la interacción con cada uno de los $N-1$ paneles restantes (aquí con subíndice $w$), \textbf{y} consigo mismo. Igual que anteriormente para una situación que considera la ecuación PB por fuera de la interfaz,
