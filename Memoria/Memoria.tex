...
\noindent
Como se adelantó en la sección anterior, las menores diferencias se obtienen cuando se fija una condición de borde tipo \texttt{zero}, lo que tiene sentido si se considera que existe una membrana definida por dos interfaces que encierran un medio distinto del agua con sal, dichas interfaces teniendo vectores normales $(0,0,\pm 1)$. Físicamente, esto significa que la condición de contorno \texttt{sdh} debiese considerar el efecto del campo de reacción debido a las interfaces para ser válido. Distinto es el caso de una condición \texttt{zero}, que solo requiere un dominio de integración grande, ya que independiente de la reacción del medio, el potencial en cualquier dirección tiende a $0$ a medida que aumenta la distancia. 
